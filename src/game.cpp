...
Game::Game(const std::uint32_t gridWidth, const std::uint32_t gridHeight, const std::uint32_t mineCount) :
    m_VertexArray(sf::PrimitiveType::Triangles, 500), 
    m_Sound(m_Audio.win), 
...
void Game::generate_mines(const std::uint32_t ignoreX,
                                                    const std::uint32_t ignoreY) {
...
void Game::handle_event(const sf::Event &event) {
    ...
    case sf::Event::MouseButtonPressed: {
        m_Window.setView(m_GridView);
        const sf::Vector2f mousePos = m_Window.mapPixelToCoords(
                sf::Vector2i(event.mouseButton.x, event.mouseButton.y));
        const sf::Vector2u cellPos(
                mousePos.x / m_GridView.getSize().x * m_GridWidth,
                mousePos.y / m_GridView.getSize().y * m_GridHeight);
    ...
    case sf::Event::Resized: {
        ...
        m_GameOverText.setScale(sf::Vector2f(1.0f, 1.0f) * m_ViewUI.getSize().x /
                                                        1000.0f);
        break;
    }
