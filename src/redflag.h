...
typedef enum { RF_BOOL, RF_INT, RF_FLOAT, RF_STR } rf_flag_type_t;
...
rf_flag_t *rf_flag_bool(rf_context_t *ctx, const char *name, const char *description);
...
rf_flag_t *rf_flag_bool(rf_context_t *ctx, const char *name, const char *description) {
	...
	flag->type = RF_BOOL;
	flag->value.as_bool = flag->default_value.as_bool = false;
	return flag;
}
